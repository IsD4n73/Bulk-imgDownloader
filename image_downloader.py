...
today = str(datetime.datetime(now.year, now.month, now.day, now.hour, now.minute, now.second)).replace(' ', '-').replace(':', '_')
...
def get_image_src(link):
    ...
    beautiful_obj = BS(response.content, 'html.parser', from_encoding="iso-8859-1")
...
def crawler(url, all_link='n'):
    ...
    if response.status_code != 200:
        print('Codice errore : %s' % response.status_code)
        exit()
                ...
                for image_src in image_srcs:
                    ...
                    if image_src not in downloaded_images:
                        ...
                        if len(downloaded_images) >= 50 and too_much == '':
                            too_much = input('Ci sono più di 50 foto vuoi continuare? <s oppure n> : ').strip() or 's'
                            if too_much[0] not in ['s', 'S', 'n', 'N']:
                                too_much = 's'
...
# ask for user input, none are present
    ...
    visit_all = input('Vuoi scansionare anche le sottopagine? <n o s> : ').strip() or 'n'


# check the url and visit_all var
if visit_all[0] not in ['S', 's', 'n', 'N']:
    visit_all = 'n'
